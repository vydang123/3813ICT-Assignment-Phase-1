...
Parameters:
•	URL Parameter:
    o	userId: The unique identifier of the user that needs to be deleted.
•	Return Values:

•	On successful deletion:
    o	message: "User deleted successfully."
•	On error reading the file:
    o	message: "Error reading the file."
•	On error writing to the file:
    o	message: "Error writing to the file."
Purpose:
...
Parameters:
•	Request Body:
    o	userId: The unique identifier of the user whose role needs to be updated.
    o	newRole: The new role that should be assigned to the user.
Return Values:

•	On successful role update:
    o	message: "User role updated successfully."
•	On error reading the file:
    o	message: "Error reading the file."
•	On error writing to the file:
    o	message: "Error writing to the file."
...
•	On successful retrieval of user data:
    o	A JSON array containing details of all users.
•	On error reading the file:
    o	message: "Error reading the file."
...
•	On successful retrieval of group data:
    o	A JSON array containing details of all groups and their associated channels.
•	On error reading the file:
    o	message: "Error reading the file."
...
Parameters:
•	Request Body:
    o	userId: The ID of the user that needs to be added to the group. This value should be a number or a string that can be converted to a number.
    o	groupId: The ID of the group to which the user will be added. This value should be a number or a string that can be converted to a number.

Return Values:
•	On successful addition of the user to the group:
    o	success: true
    o	message: "User added to group."
•	On invalid or missing parameters:
    o	success: false
    o	message: "userId and groupId are required."
•	On server errors (e.g., reading or writing to the file):
    o	success: false
    o	message: "Server error."
    o	If the specified group is not found:
    o	success: false
    o	message: "Group not found."
...
Parameters:
•	Request Body:
    o	The request body should contain the group details you want to add. The exact structure depends on the schema you've defined for a group in group-channel.json. Typically, this could include fields like groupid, groupName, members, etc.

Return Values:

•	On successful addition of the group:
    o	success: true
    o	message: "Group added successfully."
•	On server errors (e.g., reading or writing to the file):
    o	message: "Error reading the file." or "Error writing to the file."
...
Parameters:
•	Request Body:
    o	groupName: The name of the channel you want to add.
    o	groupId: The unique identifier of the group to which the channel should be added.

Return Values:
•	On successful addition of the channel to the group:
    o	success: true
    o	message: "Channel added to group."
•	On errors (e.g., missing parameters, group not found, server error during reading or writing to the file):
    o	success: false
    o	message: A corresponding error message like "groupName and groupId are required." or "Group not found." or "Server error."
...
Parameters:
•	URL Parameters:
    o	channelId: The identifier of the channel you want to remove.
    o	groupId: The unique identifier of the group from which the channel should be removed.

Return Values:
•	On successful removal of the channel from the group:
    o	success: true
    o	message: "Channel removed from group."
•	On errors (e.g., group not found, channel not found in the group, server error during reading or writing to the file):
    o	success: false
    o	message: A corresponding error message like "Group not found." or "Server error."
...
Parameters:
•	URL Parameters:
    o	userId: The unique identifier of the user you want to remove from a group.
    o	groupId: The unique identifier of the group from which you want to remove the user.

Return Values:
•	On successful removal of the user from the group:
    o	message: "User removed from group successfully."
•	On errors (e.g., user not found, server error during reading or writing to the file):
    o	message: A corresponding error message like "User not found." or "Error writing to the file."
...
•	Methods:
    o	getUsers(): Fetches all users.
    o	registerUser(userData): Registers a user with default properties and the given userData.
    o	deleteUser(userId): Deletes a user by the given user ID.
    o	updateUserRole(userId, newRole): Updates the role of a user.
    o	Error Handling: Contains a private method handleError to handle HTTP errors and logs them.
...
•	Properties:
    o	socket: Object for the Socket.io client-side connection.
•	Methods:
    o	sendMessage(message: string): Sends a chat message to the server.
    o	getMessages(): Returns an Observable that emits incoming messages.
...
•	Properties:
    o	title: Application's title.
    o	email, password: For handling login input fields.
    o	errorMessage: For showing an error message during login.
    o	userList: A hardcoded list of users to check during login (this seems like a temporary solution).
•	Methods:
    o	login(): Handles the login logic.
    o	logout(): Clears session storage and redirects to the login page.

## 2.	LoginComponent (login.component.ts & login.component.html):
•	Description: Component for handling the user login using a form.
•	Properties:
    o	userpwd: Object holding the email and password for the user trying to log in.
•	Methods:
    o	loginfunc(): Logs the user in by making an HTTP request using the httpClient. If the login is successful, it saves user details to sessionStorage and navigates to the dashboard.
...
•	Properties:
    o	userProfile: An object that holds the user's profile data, like username and user ID.
•	Methods:
    o	saveChanges(): Saves changes made by the user to sessionStorage.
...
•	Properties:
    o	newUser: Object holding the details (username, email, password) of a user to be added.
    o	users: Array storing the list of all users.
•	Methods:
    o	onSubmit(): Handles the submission of the registration form. Registers a new user via the UserService, adds the new user to the local users list, and provides feedback on success or error.
    o	fetchUserGroups(): Fetches the list of users and updates the users property.
    o	upgradeToSuperAdmin(user: any): Upgrades a user to the role of 'superadmin'.
    o	upgradeToGroupAdmin(user: any): Upgrades a user to the role of 'groupadmin'.
    o	degradeToUser(user: any): Degrades a user to the role of 'user'.
    o	updateUserRole(user: any, newRole: string): A private utility function for updating the role of a user, both locally and on the server side.
    o	deleteUser(user: any): Deletes a user both locally and on the server side using UserService.
...
•	Properties:
    o	newGroup: Object representing a new group with properties groupid, groupname, and channels.
    o	selectedUser: Holds the selected user.
    o	selectedGroup: Holds the selected group.
    o	selectedChannel: Holds the name of the selected channel.
    o	users: Array of user objects.
    o	groups: Array of group objects.
    o	newChannelName: String representing the name of a new channel.
    o	selectedGroupForChannel: Holds the group where the channel should be added.
•	Methods:
    o	ngOnInit(): Lifecycle hook that fetches all users and groups and checks user role on initialization.
    o	fetchAllUsers(): Fetches all users and sets the users property.
    o	fetchAllGroups(): Fetches all groups and sets the groups property.
    o	addGroup(): Adds a new group.
    o	addUserToGroup(user: any, group: any): Adds a user to a group.
    o	addChannelToGroup(): Adds a channel to a specified group.
    o	removeUserFromGroup(user: any, group: any): Removes a user from a group.
    o	deleteChannelFromGroup(channel: string, group: any): Deletes a channel from a specified group.
    o	deleteGroup(group: any): Deletes a specified group.
•	Dependencies:
    o	UserService: Not directly utilized in the provided code, but it's injected and might be used in a more detailed implementation.
    o	HttpClient: Used for making HTTP requests.
    o	Router: Used for navigating to other components/views.


## 6.	RegisterComponent:
•	Description: Component responsible for user registration, allowing a new user to provide a username, email, and password.
•	Properties:
    o	newUser: Object that holds the details of the user being registered which includes username, email, and password.
•	Methods:
    o	onSubmit(): This method gets triggered when the form is submitted. It invokes the registerUser method from the UserService to register a new user. Depending on the response from the server, it alerts the user about the success or failure of the registration process.
...
•	Properties:
    o	userRole: A string that captures the role of the logged-in user. This role is retrieved from the session storage and used to conditionally render content.
•	Methods:
    o	ngOnInit(): An Angular lifecycle hook that gets triggered when the component is initialized. In this method, the user's role is fetched from the session storage and set to the userRole property. If the role is not found, an error is logged to the console.

## 8.	ChatComponent:
•	Description: A component that provides chat functionality. Users can send and view messages.
•	Properties:
    o	message: A string to capture the message typed by the user.
    o	messages: An array of strings that keeps a list of messages sent.
•	Methods:
    o	sendMessage(): Sends the user's message using the ChatService. It also adds the message to the local messages list and resets the input field.


## Models:
User Model (Register):
•	Properties:
    o	username: A string representing the user's name.
    o	email: A string representing the user's email.
    o	password: A string representing the user's password.

Message Model (Chat):
•	Properties:
    o	content: A string representing the content of the message.
